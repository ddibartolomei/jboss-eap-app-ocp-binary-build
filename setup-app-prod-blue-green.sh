...
for DEPLOY_VERSION_ID in $DEPLOYMENT_VERSIONS; do
    ...
    oc rollout pause dc ${VERSIONED_APP_NAME} ${NAMESPACE_OCP_TOKEN_COMPOSITE_PARAM}
    ...
    oc delete is/${VERSIONED_APP_NAME} ${NAMESPACE_OCP_TOKEN_COMPOSITE_PARAM}
    ...
    if [[ "${DEPLOYMENT_STRATEGY_RECREATE_ENABLED}" == "true" ]]; then
        ...
        oc patch dc ${VERSIONED_APP_NAME} -p "{\"spec\":{\"strategy\":{\"type\":\"Recreate\"}}}" ${NAMESPACE_OCP_TOKEN_COMPOSITE_PARAM}
    else
    ...
    # Resume dc
    oc rollout resume dc ${VERSIONED_APP_NAME} ${NAMESPACE_OCP_TOKEN_COMPOSITE_PARAM}
